...
var DateObject = /** @class */ (function () {
    ...
    DateObject.prototype.parsePart = function (_a) {
        ...
        var parsedDate = null;
        var _b = symbol === MONTH_SYMBOL
            ? this.matchMonth(currentChar)
            : { monthName: '', matchesCount: 0 }, monthByChar = _b.monthName, matchesCount = _b.matchesCount;
        ...
        if (monthByChar) {
            ...
            if (!parsedDate && symbol === MONTH_SYMBOL) {
                // intl.parseDate may have failed because the active day in the base date
                // (default: 31 Jan 2000) exceeds the number of days in the matched month
                // (e.g. day 31 when switching to November which has only 30 days)
                // Use the same rollover approach as the numeric month parsing logic
                var monthNumber = this.monthNames.indexOf(monthByChar);
                if (monthNumber >= 0) {
                    parsedDate = (0, kendo_date_math_1.cloneDate)(this.value);
                    parsedDate.setMonth(monthNumber);
                    if (parsedDate.getMonth() !== monthNumber) {
