...
namespace TheaterAdmin.Controllers
{
    public class CategoriesController : Controller
    {
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            if (category != null)
            {
                // Guard: a category with movies attached cannot be deleted (matches the
                // NOT NULL foreign key rule on Movie.CategoryId — deleting it would either
                // orphan movies or violate the FK constraint).
                bool hasMovies = await _context.Movies.AnyAsync(m => m.CategoryId == id);
                if (hasMovies)
                {
                    ModelState.AddModelError(string.Empty, "Cannot delete this category because one or more movies are assigned to it.");
                    return View(category);
                }
